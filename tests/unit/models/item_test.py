...
class ItemTest(UnitBaseTest):
    def test_create_item(self):
        ...
        self.assertEqual(item.store_id, 1,
                         f"The store_id of the item after creation does not equal the constructor argument.")
        self.assertIsNone(item.store,
                          f"The store of the item after creation doest none as expected. item.store {item.store}")
